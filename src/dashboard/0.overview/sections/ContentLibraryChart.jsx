...
export default function ContentLibraryChart() {
  ...
  return (
    <section className="rounded-2xl border border-navy-900/8 bg-white p-6 shadow-elevated">
      ...
      {total ? (
        <>
          <p className="mb-5 mt-1 text-sm text-navy-900/50">
          ...
          <div className="flex flex-wrap items-center gap-8">
            <div className="relative shrink-0" style={{ width: SIZE, height: SIZE }}>
              ...
              <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
                ...
                <span className="text-[10px] uppercase tracking-widest text-navy-900/40">Items</span>
              </div>
            </div>
              ...
              {items.map((i) => (
                <div key={i.label} className="flex items-center justify-between gap-3 text-sm">
                  ...
                  <span className="flex items-center gap-2">
                    <span className="font-mono text-xs text-navy-900/50">{Math.round((i.value / total) * 100)}%</span>
